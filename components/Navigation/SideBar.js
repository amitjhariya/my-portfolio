...
function Sidebar ({ open, setOpen }) {
  ...
  return (
    <motion.div initial={{ x: -300 }} animate={{ x: 0 }}>
      <aside
        className={`fixed bg-[#000000] min-h-screen max-h-screen ${open ? "w-72" : "w-16"
              ...
              >
                ...
                <h2
                  className={`${open && "hidden"
                    } absolute left-48 bg-white font-semibold whitespace-pre text-gray-900 rounded-md drop-shadow-lg px-0 py-0 w-0 overflow-hidden group-hover:px-2 group-hover:py-1 group-hover:left-14 group-hover:duration-300 group-hover:w-fit  -z-100`}
                >
                  {menu?.name}
                </h2>
              </Link>
